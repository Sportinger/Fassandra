...
async function main() {
  try {
      ...
      defaultFragment.forEach((node) => {
        ...
        if (node.nodeName === 'sceneBlock') {
          ...
          if (sceneNumber) {
            ...
            // Clear and set text content to just "SZENE"
            node.delete(0, node.length);
            node.insert(0, [new Y.XmlText('SZENE')]);

            if (fixedScenes < 10) {
              console.log(`✓ Scene ${sceneNumber}: name="${newSceneName}", text="SZENE"`);
            }
            fixedScenes++;
          }
        }
    ...
    console.log(`\n✅ Complete fix:`);
    console.log(`  - Fixed ${fixedScenes} scenes (number + text content)`);
    console.log(`  - Fixed ${fixedSpeakers} dialogues (speaker names)`);

    const newUpdate = Y.encodeStateAsUpdate(doc);
    const newStateVector = Y.encodeStateVector(doc);

    await fs.writeFile('/tmp/marquise_clean_state.bin', newUpdate);
    await fs.writeFile('/tmp/marquise_clean_vector.bin', newStateVector);
