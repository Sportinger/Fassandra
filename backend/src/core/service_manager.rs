...
impl ServiceManager {
    ...
    pub async fn new(
        ...
        let (persistence_tx, persistence_rx) = mpsc::channel::<YjsPersistenceEvent>(1024);
        ...
        service_manager
            .start_background_services(persistence_rx)
            .await?;
            ...
            loop {
                ...
                match rate_limiter_cleanup.cleanup_old_data().await {
                    Ok(removed) => {
                        if removed > 0 {
                            tracing::debug!(
                                "🧹 RateLimiter cleanup: removed {} old entries",
                                removed
                            );
                        }
                    }
            ...
            loop {
                ...
                match crate::networking::cleanup_inactive_sessions(inactive_threshold).await {
                    Ok(removed_count) => {
                        if removed_count > 0 {
                            tracing::info!(
                                "🧹 WebSocket cleanup: removed {} inactive sessions",
                                removed_count
                            );
                        } else {
    ...
    pub fn get_extended_script_services(
        &self,
    ...
    fn create_script_services_internal(&self) -> crate::handlers::script::ScriptServices {
        ...
        let script_repo = std::sync::Arc::new(
            crate::repositories::script_repository::PostgresScriptRepository::new(
                self.database_pool.clone(),
            ),
        );
        let user_repo = std::sync::Arc::new(
            crate::repositories::user_repository::PostgresUserRepository::new(
                self.database_pool.clone(),
            ),
        );
        let _yjs_repo = std::sync::Arc::new(
            crate::repositories::yjs_update_repository::PostgresYjsUpdateRepository::new(
                self.database_pool.clone(),
            ),
        );

        // Create domain services layer
        let script_domain_service =
            std::sync::Arc::new(crate::domain::script_service::ScriptService::new(
                script_repo.clone(),
                user_repo.clone(),
            ));
        ...
        let script_service =
            std::sync::Arc::new(crate::application::ScriptApplicationService::new(
                script_domain_service.clone(),
                self.database_pool.clone(),
            ));

        let sharing_service =
            std::sync::Arc::new(crate::application::ScriptSharingApplicationService::new(
                script_domain_service.clone(),
                self.database_pool.clone(),
            ));

        let thumbnail_service =
            std::sync::Arc::new(crate::application::ThumbnailApplicationService::new(
                script_domain_service.clone(),
                self.database_pool.clone(),
            ));
