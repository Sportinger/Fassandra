...
async fn main() -> Result<()> {
    ...
    if let Err(e) = update_admin_user_password(&pool).await {
        tracing::warn!("Issue during admin user password update: {}. Continuing...", e);
    }
...
async fn create_database_pool(config: &Config) -> Result<sqlx::PgPool> {
    ...
    tracing::info!("✅ Database connection pool created (max_connections: {})", config.db_max_connections);
    Ok(pool)
}
