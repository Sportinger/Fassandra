...
impl BlockRepository for PostgresBlockRepository {
    async fn find_by_script_id(&self, script_id: Uuid) -> Result<Vec<Block>, AppError> {
        let blocks = sqlx::query_as!(
            Block,
            "SELECT id, script_id, block_type, content, block_order, page_number, created_at FROM blocks WHERE script_id = $1 ORDER BY block_order ASC",
            script_id
        )
    ...
    async fn create(&self, block: &Block) -> Result<(), AppError> {
        sqlx::query!(
            "INSERT INTO blocks (id, script_id, block_type, content, block_order, page_number, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
            block.id,
            block.script_id,
            block.block_type,
            block.content,
    ...
    async fn update(&self, block: &Block) -> Result<(), AppError> {
        sqlx::query!(
            "UPDATE blocks SET block_type = $2, content = $3, block_order = $4, page_number = $5 WHERE id = $1",
            block.id,
            block.block_type,
            block.content,
            block.block_order,
