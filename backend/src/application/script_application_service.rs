...
use yrs::GetString;
use yrs::{ReadTxn, Text, WriteTxn, XmlFragment as _};
...
impl ScriptApplicationService {
    ...
    pub async fn create_script_from_parsed(
        ...
        self.script_service
            .validate_script_creation(user_id)
            .await?;

        // Create the script with all business rules applied
        let script_id = self
            .create_script_from_parsed_internal(parsed_script, user_id)
            .await?;
    ...
    async fn create_script_from_parsed_internal(
        ...
        let script_title = if let Some(source_filename) = &parsed_script.source_filename {
            ...
            Self::extract_main_title(parsed_script.title.as_deref().unwrap_or("Untitled Script"))
        };
        ...
        for (section_index, section) in parsed_script.sections.iter().enumerate() {
            for (element_index, element) in section.content.iter().enumerate() {
                ...
                let block_type = match element {
                    ...
                    crate::analysis::structs::ContentElement::StageDirection(_) => {
                        "stage_direction"
                    }
                ...
                for element in &section.content {
                    ...
                    match element {
                        ...
                        crate::analysis::structs::ContentElement::Dialogue(d) => {
                            let dlg_ref = default_fragment
                                .push_back(&mut txn, XmlElementPrelim::empty("dialogueBlock"));
                            let sp_ref =
                                dlg_ref.push_back(&mut txn, XmlElementPrelim::empty("speaker"));
                            if let Some(spk) = &d.speaker {
                                sp_ref.push_back(&mut txn, XmlTextPrelim::new(spk.clone()));
                            }
                            let dtext_ref = dlg_ref
                                .push_back(&mut txn, XmlElementPrelim::empty("dialogueText"));
                        ...
                        crate::analysis::structs::ContentElement::StageDirection(sd) => {
                            ...
                            if let Some(t) = &sd.description {
                                text = t.clone();
                            }
                            if text.is_empty() {
                                if let Some(t) = &sd.line {
                                    text = t.clone();
                                }
                            }
                            if text.is_empty() {
                                if let Some(t) = &sd.reading_text {
                                    text = t.clone();
                                }
                            }
                            if !text.is_empty() {
                                let p = default_fragment
                                    .push_back(&mut txn, XmlElementPrelim::empty("paragraph"));
                        ...
                        crate::analysis::structs::ContentElement::Reading(r) => {
                            let mut text = String::new();
                            if let Some(t) = &r.reading_text {
                                text = t.clone();
                            }
                            if text.is_empty() {
                                if let Some(t) = &r.description {
                                    text = t.clone();
                                }
                            }
                            if text.is_empty() {
                                if let Some(t) = &r.line {
                                    text = t.clone();
                                }
                            }
                            if !text.is_empty() {
    ...
    pub async fn get_script_with_yjs(
        ...
        match crate::services::yjs_compaction_service::load_document(self.pool.as_ref(), script_id)
            .await
        {
            Ok(doc) => {
            ...
            Err(e) => {
                error!(
                    "Failed to load YJS document for script {}: {}",
                    script_id, e
                );
                // Create a proper empty YJS document state instead of empty vec
                use yrs::{Doc, Options, ReadTxn, Transact, WriteTxn};
                let doc = Doc::with_options(Options::default());
                ...
                let state = doc
                    ...
                    .encode_state_as_update_v1(&yrs::StateVector::default());
                info!(
                    "Created fallback empty YJS state for script {} (size: {})",
                    script_id,
                    state.len()
                );
                Ok(Some((script, state)))
